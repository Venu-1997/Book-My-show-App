...
const MovieList = () => {
    ...
    const loadMovies = async () => {
        const res = await fetch('http://localhost:5050/api/movies/list?type=ALL');
                                    ...
                                    return (
                                        <li className="flex justify-between gap-x-6 py-5">
                                            ...
                                            <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                                                ...
                                                <p className="mt-1 text-xs leading-5 text-gray-500">Genre : 
                                                {
                                                    movie.genre.map((gen) => {
                                                        return(
                                                            <span className="text-s font-bold tracking-tight"> {gen} /</span>
                                                        )
                                                    })
                                                }
